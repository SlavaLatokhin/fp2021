...
module ClassLoader (M : MONADERROR) = struct
  ...
  let begin_inheritance_from_exception ht =
    get_elem ht "Exception" >>= fun ex_cl ->
  ...
  let load_classes class_list class_table =
    ...
    | _ ->
        system_exception_init class_table >>= fun table ->
        class_adding class_list table >>= fun table_with_classes ->
        add_default_constructor table_with_classes >>= fun table_update ->
        update_exception_class_childs table_update >>= fun new_table ->
        begin_inheritance_from_exception new_table
end
...
module Interpreter (M : MONADERROR) = struct
  ...
  let rec check_expr_type cur_expr ctx class_table =
    match cur_expr with
    | Plus (left, right) -> (
        check_expr_type left ctx class_table >>= fun left_type ->
        match left_type with
        | Int -> (
            check_expr_type right ctx class_table >>= fun right_type ->
    ...
    | PostDec value | PostInc value | PrefDec value | PrefInc value -> (
        check_expr_type value ctx class_table >>= fun value_type ->
    ...
    | CallMethod (method_name, args) ->
        let obj_key =
          ...
          | ObjNull -> "null"
          | ObjRef { class_key = key; _ } -> key
        in
        get_elem class_table obj_key >>= fun curr_class ->
    ...
    | Access (calling_obj, IdentVar var_name) -> (
        check_expr_type calling_obj ctx class_table >>= fun obj_type ->
        match obj_type with
        | CsClass "null" -> error "NullReferenceException"
        | CsClass obj_key -> (
            get_elem class_table obj_key >>= fun found_obj ->
            let find_field =
              get_elem_if_present found_obj.field_table var_name
            in
    ...
    | IdentVar var_key -> (
        ...
        | None -> (
            ...
            | ObjRef { class_table = table; _ } -> (
                ...
                | None ->
                    error "There is no such variable or field with that name!"
  ...
  and check_constructor_type t_class args g_ctx class_table =
    ...
    let rec check_helper ht pos expr_list ctx =
      ...
      | other -> (
          ...
          | x :: xs ->
              check_expr_type x ctx class_table >>= fun x_type ->
              check_helper
    ...
    let delete : key_t -> variable -> unit =
     fun key element ->
      if element.visibility_level = ctx.visibility_level then
        Hashtbl.remove ctx.variable_table key
    in
  ...
  let rec eval_stat stat input_ctx class_table =
        ...
        let rec eval_stat_bl : statement list -> context -> context M.t =
         fun stl hctx ->
          ...
          | st :: tail -> (
              ...
              | _ ->
                  eval_stat st hctx class_table >>= fun head_ctx ->
                  eval_stat_bl tail head_ctx)
        in
        eval_stat_bl stat_list input_ctx >>= fun new_ctx ->
    ...
    | While (smexpr, stat) -> (
        ...
        let rec loop l_stat ctx =
          ...
          if ctx.runtime_signal = WasBreak then
            ...
            eval_expr smexpr ctx class_table >>= fun new_ctx ->
            ...
            | VBool true -> (
                eval_stat l_stat new_ctx class_table >>= fun l_ctx ->
    ...
    | If (smexpr, then_stat, else_stat) -> (
        eval_expr smexpr input_ctx class_table >>= fun new_ctx ->
    ...
    | For (dec_stat_o, smexpr_o, after_list, body_stat_o) ->
        ...
        | None ->
            return (inc_visibility_level { input_ctx with is_main = false })
        | Some dec_stat ->
            eval_stat dec_stat
        ...
        let rec loop body_stat af_list ctx =
          ...
          if ctx.runtime_signal = WasBreak then
            ...
            | VBool true -> (
                let rec inter_expr_list e_list as_ctx =
                  ...
                  | x :: xs ->
                      if expr_in_stat x then
                        eval_expr x as_ctx class_table >>= fun z_ctx ->
                        inter_expr_list xs z_ctx
                ...
                (* we can have continue - so we cycle again*)
                | WasContinue ->
                    inter_expr_list af_list body_ctx >>= fun after_ctx ->
                    loop body_stat af_list
                      { after_ctx with runtime_signal = NoSignal }
                | _ ->
                    inter_expr_list af_list body_ctx >>= fun after_ctx ->
                    loop body_stat af_list after_ctx)
    ...
    | Return (Some rexpr) ->
        check_expr_type rexpr input_ctx class_table >>= fun ret_type ->
        if ret_type <> input_ctx.current_method_type then
          ...
          eval_expr rexpr input_ctx class_table >>= fun new_ctx ->
          return { new_ctx with runtime_signal = WasReturn }
    | Expression sexpr ->
        if expr_in_stat sexpr then
          eval_expr sexpr input_ctx class_table >>= fun new_ctx ->
          return new_ctx
        ...
        let rec var_declarator var_list var_ctx =
          ...
          | (var_name, var_expr_o) :: tail -> (
              ...
              | ObjRef { class_table = table; _ } ->
                  ...
                  then error "Variable with this name is already defined"
                    ...
                    | Some var_expr_e -> (
                        ...
                        | CsClass "null" -> (
                            ...
                            | _ ->
                                error
                                  "Incorrect assign type in variable \
                                   declaration")
                  ...
                  >>= fun head_ctx -> var_declarator tail head_ctx)
        in
    ...
    | Throw sexpr -> (
        ...
        | VClass ex_cl -> (
            ...
            | ObjRef ex_obj -> (
                ...
                | Some "Exception" ->
                    return { new_ctx with runtime_signal = WasThrown }
                | None -> (
                    ...
                    | _ ->
                        error
                          "Cannot implicitly convert type to System.Exception")
                | _ ->
                    error "Cannot implicitly convert type to System.Exception"))
    ...
    | Print print_expr ->
        ...
        let printer = function
          ...
          | VNull -> error "Impossible to print null"
        in
  ...
  and eval_expr in_expr in_ctx class_table =
    let eval_helper e_expr ctx =
      let eval_op left_expr right_expr operator =
        eval_expr left_expr ctx class_table >>= fun left_ctx ->
        eval_expr right_expr left_ctx class_table >>= fun right_ctx ->
      ...
      let eval_unar ex_operand operator =
        eval_expr ex_operand ctx class_table >>= fun new_ctx ->
      ...
      let ( %% ) left right =
        ...
        | _, _ ->
            raise
              (Invalid_argument "Incorrect argument types for mod operator!")
      in
      let ( >>> ) left right =
        match (left, right) with
        | VInt x, VInt y -> VBool (x > y)
        | _ -> raise (Invalid_argument "Incorrect type for >!")
      in
      ...
      let ( === ) left right =
        ...
        | VClass x, VClass y -> (
            ...
            | ObjRef { number = xn; _ }, ObjRef { number = yn; _ } ->
                VBool (xn = yn))
      ...
      | IdentVar var_id -> (
          ...
          | None -> (
              try
                get_obj_info ctx.current_o |> fun (_, table, _) ->
                match get_elem_if_present table var_id with
                | Some field ->
                    return { ctx with last_expr_result = field.field_value }
      ...
      | Access (obj_expr, IdentVar field_key) -> (
          eval_expr obj_expr ctx class_table >>= fun ob_ctx ->
          let get_obj = ob_ctx.last_expr_result in
          match get_obj with
          | VClass (ObjRef { class_table = table; _ }) ->
              get_elem table field_key >>= fun get_field ->
      ...
      | CallMethod (m_name, args) -> (
          let get_curr_o =
            return { ctx with last_expr_result = VClass ctx.current_o }
          in
          get_curr_o >>= fun ob_ctx ->
          match ob_ctx.last_expr_result with
          | VClass ObjNull -> error "NullReferenceException"
          | VClass
              (ObjRef
      ...
      | ClassCreate (class_name, c_args) ->
          get_elem class_table class_name >>= fun get_obj ->
          ...
          let rec create_obj par_class init_ctx =
            ...
            let rec create_helper inhelp_ht rec_ctx = function
              ...
              | (cur_field_type, field_name, field_expr_o) :: tail ->
                  let is_const f_key =
                    get_elem get_obj.field_table f_key >>= fun find_t_field ->
                    return find_t_field.is_const
                  in
                  (match field_expr_o with
                  | Some f_expr -> (
                      check_expr_type f_expr rec_ctx class_table
                      >>= fun expr_type ->
                      is_const field_name >>= fun const_res ->
                      let add_field fe =
                        eval_expr fe rec_ctx class_table >>= fun fe_ctx ->
                  ...
                  obj_num head_ctx.current_o >>= fun num ->
            ...
            | Some par_key ->
                get_elem class_table par_key >>= fun parent_r ->
                create_obj parent_r init_ctx >>= fun par_ctx ->
                create_helper
          ...
          prepare_constructor_block found_constr.body get_obj >>= fun c_body ->
          eval_stat c_body
            {
              initres_ctx with
              variable_table = vt;
      ...
      | _ -> error "Incorrect expression!"
    in
    check_expr_type in_expr in_ctx class_table >>= fun _ ->
  ...
  and update_table ht args meth_args ctx class_table =
    fold_left2
      (fun (new_ht, hctx) arg pair ->
        match pair with
        | h_type, h_name ->
            eval_expr arg hctx class_table >>= fun new_ctx ->
  ...
  and update_identifier var_key value var_ctx =
    if Hashtbl.mem var_ctx.variable_table var_key then (
      get_elem var_ctx.variable_table var_key >>= fun get_old ->
      check_assign_variable get_old >>= fun _ ->
      ...
      | ObjRef { class_table = table; _ } ->
          if Hashtbl.mem table var_key then
            get_elem table var_key >>= fun get_old ->
            check_assign_field get_old >>= fun _ ->
  ...
  and update_field obj_expr field_name field_ctx class_table =
    eval_expr obj_expr field_ctx class_table >>= fun obj_evaled_ctx ->
    let get_obj = get_obj_value obj_evaled_ctx.last_expr_result in
    let cal_new_val = field_ctx.last_expr_result in
    try
      get_obj_info get_obj |> fun (_, fl_table, _) ->
      if Hashtbl.mem fl_table field_name then
        get_elem fl_table field_name >>= fun old_field ->
        check_assign_field old_field >>= fun _ ->
  ...
  and update_object obj field_key value up_ctx =
    ...
    in
    get_obj_info obj |> fun (_, object_frt, object_number) ->
  ...
  let start_interpreting : (key_t, class_t) Hashtbl.t -> context M.t =
   fun ht ->
    find_main_class ht >>= fun main_class ->
    init_contex
      (ObjRef
         {
    ...
    eval_stat main.body ctx ht >>= fun final_ctx ->
