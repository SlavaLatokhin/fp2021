...
module Expression = struct
  ...
  and call_method input =
    ( ident_object >>= fun name ->
      token "(" >> split_by_comma >>= fun args_list ->
      token ")" >> return (CallMethod (name, args_list)) )
      input

  and init_instance input =
    ( token "new" >> ident_object >>= fun name ->
      token "(" >> split_by_comma >>= fun args_list ->
  ...
  and field_access input =
    ...
    ( helper >>= fun head ->
      many1 (token "." >> helper) => fun tl ->
      List.fold_left (fun head tl -> Access (head, tl)) head tl )
      input
...
module Statement = struct
  ...
  and if_stat input =
    ( token "if" >> parens expr >>= fun condition ->
      parse_statements >>= fun if_body ->
      choice
        [
          ( token "else" >> parse_statements >>= fun else_body ->
  ...
  and statement_block input =
    ( braces (sep_by parse_statements spaces) >>= fun stats ->
      return (StatementBlock stats) )
      input

  and while_stat input =
    ( token "while" >> parens expr >>= fun condition ->
      parse_statements >>= fun while_body ->
      return (While (condition, while_body)) )
      input

  and var_declare input =
    let helper =
      ident_object >>= fun var_name ->
      token "=" >> expr
  ...
  and for_stat input =
    ...
    >>= fun declare ->
      ...
      token ")" >> parse_statements >>= fun body ->
      return (For (declare, condition, after, body)) )
      input

  and throw input =
    ( token "throw" >> expr >>= fun throw_expr ->
  ...
  and try_stat input =
    ...
    *)
    let catch =
      ...
      >> choice
           [
             ( token "(" >> Expression.define_type >>= fun excep_type ->
               choice
                 [
               ...
               >>= fun var_name ->
               token ")" >> filter >>= fun filter ->
               parse_statements >>= fun catch_block ->
               return (Some (excep_type, var_name), filter, catch_block) );
             ( filter >>= fun filter ->
               parse_statements >>= fun catch_block ->
               return (None, filter, catch_block) );
           ]
    in
    ( token "try" >> parse_statements >>= fun try_stat ->
      many catch >>= fun catch_list ->
      match catch_list with
      | [] ->
          token "finally" >> parse_statements >>= fun finally_block ->
  ...
  and print_cs input =
    ( token "Console.WriteLine(" >> expr >>= fun print_expression ->
      token ");" >> return (Print print_expression) )
      input
end

let get_params =
  Expression.define_type >>= fun _type ->
  Expression.ident_object >>= fun name -> return (_type, name)

let class_method =
  Expression.define_type >>= fun method_type ->
  Expression.ident_object >>= fun method_name ->
  token "(" >> sep_by get_params (token ",") >>= fun params_list ->
  token ")" >> Statement.statement_block >>= fun stat_block ->
  return (Method (method_type, method_name, params_list, stat_block))

let constructor =
  Expression.ident_object >>= fun name ->
  token "(" >> sep_by get_params (token ",") >>= fun params_list ->
  token ")" >> Statement.statement_block >>= fun stat_block ->
  return (Constructor (name, params_list, stat_block))
...
let field =
  let helper =
    ...
    <|> return (name, None)
  in
  Expression.define_type >>= fun f_type ->
  sep_by helper (token ",") >>= fun var_list ->
  token ";" >> return (VariableField (f_type, var_list))

let class_elements =
  modifier_list >>= fun modifiers ->
  field <|> class_method <|> constructor >>= fun class_elem ->
  return (modifiers, class_elem)

let parse_class =
  modifier_list >>= fun modifiers ->
  token "class" >> Expression.ident_object >>= fun name ->
  choice
    [
      ( token ":" >> Expression.ident_object >>= fun parent ->
        return (Some parent) );
      return None;
    ]
  >>= fun _parent ->
  token "{" >> sep_by class_elements spaces >>= fun class_elements ->
